...
\textbf{Diagram klas} przedstawia strukturę pakietów aplikacji:

\begin{itemize}
    \item \texttt{pl.tablicaogloszen.wspolne} -- klasy DTO (Data Transfer Objects) współdzielone między klientem a serwerem
    \item \texttt{pl.tablicaogloszen.serwer} -- logika serwera, DAO, zarządzanie połączeniami
    \item \texttt{pl.tablicaogloszen.klient} -- aplikacja klienta JavaFX, kontrolery FXML
\end{itemize}

Główne klasy modelu:
\begin{itemize}
    \item \texttt{OgloszenieDTO} -- reprezentacja ogłoszenia (id, tytuł, treść, kategoria, autor, data, wyświetlenia, zgłoszenia)
    \item \texttt{UzytkownikDTO} -- reprezentacja użytkownika (id, login, rola)
    \item \texttt{FiltrDTO} -- parametry filtrowania i sortowania
    \item \texttt{Zadanie} -- żądanie klienta (typ + dane)
    \item \texttt{Odpowiedz} -- odpowiedź serwera (status + dane + wiadomość)
\end{itemize}

\textbf{Diagram przypadków użycia} pokazuje interakcje aktorów z systemem:
\begin{itemize}
    \item \textbf{Użytkownik niezalogowany}: rejestracja, logowanie, przeglądanie ogłoszeń
    \item \textbf{Użytkownik zalogowany}: wszystkie powyższe + dodawanie, edycja, usuwanie własnych, zgłaszanie
    \item \textbf{Administrator}: wszystkie powyższe + usuwanie dowolnych, przeglądanie zgłoszonych, raporty
\end{itemize}

\textit{Pełna dokumentacja UML znajduje się w pliku \texttt{dokumentacja.puml} w formacie PlantUML.}
